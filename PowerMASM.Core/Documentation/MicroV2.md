...
## Functions
Function's are PowerMASM's extension to MicroASM, and are not supported in all runtimes.

### DEF (Define Function)
```
FDEF <TYPE> <name>(arg1, arg2, ...) <SCOPE?> {}
```
Defines a function with the specified name and arguments. Optionally, the function can be defined within a named scope.
-   `FDEF`: Keyword to define a function.
-   `<TYPE>`: The return type of the function. Supported types are:
    -   `<VOID>`: No return value.
    -   `<BYTE>`: 8-bit integer return value.
    -   `<WORD>`: 16-bit integer return value.
    -   `<DWORD>`: 32-bit integer return value.
    -   `<QWORD>`: 64-bit integer return value.
    -   `<FLOAT>`: 32-bit floating-point return value.
    -   `<DOUBLE>`: 64-bit floating-point return value.
    -   `<PTR>`: Pointer/address return value (MASM Runtime / architecture-dependent size).
    -   `<STRING>`: Pointer to a null-terminated string (MASM Runtime / architecture-dependent size).
-   `name`: The name of the function. Case-sensitive.
-   `(arg1, arg2, ...)`: A comma-separated list of argument names. These names are used as local variables within the function.
-   `<SCOPE?>`: (Optional) If provided, the function is defined within the specified named scope. See Label Scoping section for details.

Functions allow developers to group related instructions and reuse them throughout the program in more extensive ways than labels and macros.

Functions allow developers to call Micro-ASM code from C# and vice versa as well as having proper handling of typed arguments.
...
if a instruction tries to use a state variable in an incompatible way (e.g., adding a `<BYTE>` to a `<QWORD>`), the assembler should raise an error.
to combat this, either use the appropriate `MOVZX`/`MOVSX` instructions to extend smaller types to larger registers, or use PowerMASM's object type-casting powers.

```
STATE small_val <BYTE> 100
STATE large_val <QWORD> 1000
mov large_val small_val ; ERROR: Type mismatch
mov large_val (QWORD)small_val ; OK: Cast small_val to QWORD before moving
```

not everything can be cast, such as casting specific Dotnet types to MicroASM types, such as casting a string to a qword without proper handling.
